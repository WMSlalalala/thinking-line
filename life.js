...
function mapMarkup(){return '<div class="travel-map-wrap"><div id="travel-map" class="travel-map" aria-label="Interactive map of three United States road trips"></div><div class="map-cameras"><button type="button" data-camera="usa">US overview</button></div></div>'}
...
function tripCardHTML(trip){return '<button type="button" class="trip-card'+(LIFE.selected===trip.id?' on':'')+'" data-open-trip="'+esc(trip.id)+'" aria-pressed="'+String(LIFE.selected===trip.id)+'" style="--trip-color:'+routeColor(trip)+'"><span><strong><i class="trip-swatch"></i>'+esc(trip.title)+'</strong><small>'+trip.points.slice(0,-1).map(function(p){return esc(p.name||p.label)}).join(' · ')+'</small></span></button>'}
function renderTrips(){var area=document.getElementById('life-content');area.innerHTML='<div class="life-toolbar"><h2 class="h">Road trips</h2></div>'+mapMarkup()+'<div class="trip-list" id="trip-list">'+LIFE.trips.map(tripCardHTML).join('')+'</div><p class="map-status" id="life-status" role="status"></p><section class="trip-detail" id="trip-detail"></section>';area.onclick=handleLifeClick}
...
function selectTrip(id){
  ...
  document.getElementById('trip-list').innerHTML=LIFE.trips.map(tripCardHTML).join('');
...
function handleLifeClick(e){
  ...
  if(e.target.closest('[data-camera]')){LIFE.selected=null;document.getElementById('trip-list').innerHTML=LIFE.trips.map(tripCardHTML).join('');document.getElementById('trip-detail').innerHTML='';moveCamera('usa');updateMapData()}
}
